        ...
        ((this.$element = a(c)),
          ...
          a(document).data("editable-handlers-attached") ||
            (a(document).on("keyup.editable", function (b) {
              27 === b.which &&
                a(".editable-open").editableContainer("hide", "cancel");
            }),
            a(document).on("mousedown.editable", function (c) {
              var d,
                e = a(c.target),
                f = [
                  ".editable-container",
        ...
        ((this.options.selector = !1),
          ...
          this.$element.on(
            ...
            a.proxy(function (c) {
              ...
              d.data("editable") ||
                (d.hasClass(this.options.emptyclass) && d.empty(),
          ...
          a.each(this.map, function (a, b) {
            if ("ampm" !== a) {
              if (e["$" + a]) d[a] = parseInt(e["$" + a].val(), 10);
                ...
                var c;
                ((c = e.datetime ? e.datetime[b[1]]() : "day" === a ? 1 : 0),
                  (d[a] = c));
              }
  ...
  (function (a) {
    ...
    (a.extend(a.fn.editableform.Constructor.prototype, {
      ...
      initInput: function () {
        b.apply(this);
        var c = "sm";
        this.input.options.inputsize && (c = this.input.options.inputsize);
        var d =
          "text,textarea,password,email,url,tel,number,range,time,typeaheadjs".split(
            ",",
          );
        (~a.inArray(this.input.type, d) &&
          (this.input.$input.addClass("form-control"),
          "sm" === c && this.input.$input.addClass("form-control-sm"),
          "lg" === c && this.input.$input.addClass("form-control-lg")),
          ~a.inArray(this.input.type, ["select"]) &&
            (this.input.$input.addClass("form-select"),
            "sm" === c && this.input.$input.addClass("form-select-sm"),
            "lg" === c && this.input.$input.addClass("form-select-lg")));
        var e = this.$form.find(".editable-buttons");
        ("sm" === c && e.find("button").addClass("btn-sm"),
          "lg" === c && e.find("button").addClass("btn-lg"));
      },
    }),
      (a.fn.editableform.buttons =
        '<button type="submit" class="btn btn-primary editable-submit"><i class="fa fa-check" aria-hidden="true"></i></button><button type="button" class="btn btn-default editable-cancel"><i class="fa fa-times" aria-hidden="true"></i></button>'),
    ...
    a.extend(a.fn.editableContainer.Popup.prototype, {
      ...
      call: function () {
        return (
          a(this.$element).data(this.containerDataName) ||
            a(this.$element).data(
              this.containerDataName,
              bootstrap[
                this.containerName.replace(
                  this.containerName[0],
                  this.containerName[0].toUpperCase(),
                )
              ].getOrCreateInstance(this.$element, this.containerOptions),
            ),
          this.$element[this.containerName].apply(this.$element, arguments)
        );
      },
